...
function ApprovedPage() {
  ...
  const fetchContents = async () => {
    setLoading(true);
    try {
      const res = await contentAPI.list('approved');
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      render: (by, record) => (
        record.auto_review_result === 'auto_pass'
          ? <Tag color="green">自动通过</Tag>
          : <Tag color="blue">人工审核</Tag>
      )
    },
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
        <span>共 {contents.length} 条已通过内容</span>
      ...
      <Table
        ...
        rowKey="id"
        loading={loading}
        locale={{ emptyText: <Empty description="暂无已通过内容" /> }}
      />
    </div>
  );
}
